...
import { CriarSetorOutput } from "@/core/schemas/setores/create_setor_schema";
import ServerActionResult from "@/core/types/server_action_result";

export async function criarSetorAction({
  orgaoId,
  ...data
}: CriarSetorOutput): Promise<ServerActionResult<{ ok: true }>> {
  try {
    await api.auth.post(`/api/orgaos/${orgaoId}/setores`, {
