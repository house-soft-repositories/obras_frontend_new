...
import { criarClassificacaoSchema, type CriarClassificacaoOutput } from "@/core/schemas/cadastros/create_classificacao_schema";
import ServerActionResult from "@/core/types/server_action_result";

export async function criarClassificacaoAction(input: CriarClassificacaoOutput): Promise<ServerActionResult<{ ok: true }>> {
  const parsed = criarClassificacaoSchema.safeParse(input);
  if (!parsed.success) {
    const message = parsed.error.issues[0]?.message ?? "Dados inválidos.";
    return { success: false, data: null, error: message };
  }
  try {
    await api.auth.post("/api/cadastros/classificacoes", {
